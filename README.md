...
- [ ] `Radiation` to use pre-computed distance matrix from `Network` or its children + vectorized operations for its calculations
- [ ] Update UML diagram to reflect new `Network` classes and revisions to `Hub` and `Radiation` classes